...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const processImages = () => {
      ...
      Object.entries(imageCategories).forEach(([categoryName, imageIds]) => {
        ...
        imageIds.forEach(id => {
          // 找到对应的图片并设置分类
          const imageToUpdate = imagesWithCategories.find(img => img.id === id);
          if (imageToUpdate) {
            // 使用明确的类型断言
            (imageToUpdate as unknown as { category: string }).category = categoryName;
          }
        });
      });
  ...
  const filteredImages = useMemo(() => {
    ...
    return processedImages.filter(image => (image as unknown as { category: string }).category === selectedCategory);
        ...
        <main className="flex-1 md:ml-6"> 
          ...
          {isMobile && (
            <div className="mb-4 flex items-center">
              <span className="text-gray-600 dark:text-gray-300 mr-2">当前分类:</span>
              <span className="font-medium text-blue-500">{selectedCategory || '全部'}</span>
              <span className="text-sm text-gray-500 ml-2">({getCategoryCount(selectedCategory)} 张)</span>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 md:gap-8">
            {filteredImages.slice().reverse().map((image) => (
              <div key={image.id} className="block group relative pb-[120%]">

...
function getExistingImages() {
  try {
    ...
    const startLineRegex = /^\s*export\s+const\s+images:\s*ImageItem\s*\[\s*\]\s*=/m;
    const startLineMatch = content.match(startLineRegex);

    if (!startLineMatch) {
      console.log('[警告] 在 images.ts 文件中未找到 "export const images: ImageItem[] =" 定义行');
      return [];
    }

    const definitionLineIndex = startLineMatch.index;
    const contentAfterDefinition = content.substring(definitionLineIndex + startLineMatch[0].length);
    ...
    // 从第一个 '[' 之后开始扫描
    const scanStartIndex = definitionLineIndex + startLineMatch[0].length + startIndex + 1;
    ...
    for (let i = scanStartIndex; i < content.length; i++) {
        ...
        // 简单处理字符串和注释中的括号（可能不完美，但对纯数据对象够用）
            ...
            i++; // 跳过引号本身
            while (i < content.length && (content[i] !== quoteChar || content[i-1] === '\\')) {
                i++; // 跳过字符串内容，处理转义字符
            }
    ...
    try {
      ...
      console.log(`[信息] 成功解析 images.ts，找到 ${existingImages.length} 条现有图片数据。`);
    ...
    console.log(`[警告] 读取或处理 ${OUTPUT_TS} 文件时出错: ${error.message}。将创建新文件`);
    return [];
  }
}
...
function compareAndMerge(originalExistingImages, newRawImages) {
  ...
  if (originalExistingImages && originalExistingImages.length > 0) {
    originalExistingImages.forEach(img => {
      if (img.imagePath) { // Only map if imagePath exists
        existingImageMap.set(img.imagePath, { ...img }); // Store a copy
        nextId = Math.max(nextId, img.id + 1);
      } else {
        console.warn(`[警告] 现有图片缺少 imagePath，将无法匹配: ID=${img.id}, Title=${img.title}`);
      }
    });
  }
  console.log(`[调试] 现有图片映射完成，Map size: ${existingImageMap.size}, nextId: ${nextId}`);
  ...
  newRawImages.forEach(rawImg => {
    if (!rawImg.imagePath) {
      console.warn(`[警告] 新提取的案例缺少 imagePath，将被跳过: Title=${rawImg.title}`);
      return; // Skip items without an image path
    }
     if (processedNewImagePaths.has(rawImg.imagePath)) {
        console.warn(`[警告] 在新提取的数据中发现重复的 imagePath，将跳过后续重复项: ${rawImg.imagePath}`);
        return;
    }
  ...
  removedCount = existingImageMap.size;
   existingImageMap.forEach((removedImg) => {
       console.log(`[调试] 标记为删除的图片: ID=${removedImg.id}, Path=${removedImg.imagePath}`);
  });
  ...
  console.log(`[信息] 比较合并完成. 新增: ${addedCount}, 删除: ${removedCount}, 最终: ${finalImages.length}`);
...
function updateRepository() {
  ...
  if (!fs.existsSync(REPO_DIR)) {
    ...
    console.warn('[警告] 请先手动克隆仓库: git clone https://github.com/jamez-bondos/awesome-gpt4o-images.git .temp/awesome-gpt4o-images');
  ...
  try {
    ...
    const output = execSync('git pull', { cwd: REPO_DIR, encoding: 'utf-8', env: { ...process.env, GIT_TERMINAL_PROMPT: '0' } });
...
async function main() {
  ...
  const { finalImages, addedCount, removedCount } = compareAndMerge(originalExistingImages, newRawImages);
